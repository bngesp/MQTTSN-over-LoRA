...
_response_retries(0)
{
    ...
    this->loraConnection = sx1276;
}

MQTTSN::MQTTSN(SX1276 &loraConnection) : 
waiting_for_response(false),
_connected(false),
_message_id(0),
topic_count(0),
...
//#ifdef USE_LORA
void MQTTSN::parse_rf12(uint8_t* buf, uint16_t len) {
    memcpy(response_buffer, (const void*)buf, len);
    dispatch();
}
//#endif
...
//#ifdef USE_LORA
        ...
        this->loraConnection.sendPacketTimeout(LORA_GATEWAY, message_buffer, hdr->length);
...
void MQTTSN::setLoraConnection(SX1276 &lora){
